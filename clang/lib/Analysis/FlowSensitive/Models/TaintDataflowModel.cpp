...
#include "clang/Analysis/FlowSensitive/Models/TaintDataflowModel.h"
...
ignorableOptional(const TaintDataflowModelOptions &Options) {
...
auto buildTransferMatchSwitch(
    const TaintDataflowModelOptions &Options) {
...
TaintDataflowModel::variableDecl() {
  // TODO change this
  return optionalClass();
}

TaintDataflowModel::TaintDataflowModel(
    ASTContext &Ctx, TaintDataflowModelOptions Options)
    : DataflowAnalysis<TaintDataflowModel, NoopLattice>(Ctx),
      TransferMatchSwitch(buildTransferMatchSwitch(Options)) {}

void TaintDataflowModel::transfer(const CFGElement *Elt,
                                            NoopLattice &L, Environment &Env) {
  LatticeTransferState State(L, Env);
  TransferMatchSwitch(*Elt, getASTContext(), State);
}

bool TaintDataflowModel::compareEquivalent(QualType Type,
                                                     const Value &Val1,
...
bool TaintDataflowModel::merge(QualType Type, const Value &Val1,
                                         const Environment &Env1,
                                         const Value &Val2,
                                         const Environment &Env2,
                                         Value &MergedVal,
...
TaintDataflowDiagnoser::TaintDataflowDiagnoser(
    TaintDataflowModelOptions Options)
    : DiagnoseMatchSwitch(buildDiagnoseMatchSwitch(Options)) {}

std::vector<SourceLocation> TaintDataflowDiagnoser::diagnose(
